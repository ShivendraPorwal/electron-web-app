...
export class DwtComponent implements OnInit, OnDestroy {
   ...
   */
  protected DWTObject: WebTwain = null as unknown as WebTwain;
  protected VideoContainer: WebTwain | null = null;
  /**
  ...
  ngOnInit() {
    this.eventsSubscription = this.events.subscribe((args: any) => {
      if (args.type)
        switch (args.type) {
          case 'resize':
            if (this.editorShown) break;
            else if (this.updateViewer()) {
              this.showRects();
            }
            break;
          default:
    ...
    this.barcodeReadingSubscription = this.barcodeResult.subscribe((result) => {
      if (result.done) {
              ...
              for (let i = 0; i < input.length; i++) {
                if (input[i].current) {
                  ...
                  for (let j = 0; j < this.currentOptionItems.length; j++) {
                    if ((this.currentOptionItems[j] as any).checked) {
                      this.currentItem = (
                        this.currentOptionItems[j] as any
                      ).value;
                    }
                  }
                  break;
  ...
  ngOnDestroy() {
    ...
    this.dwtService
      .unMountDWT()
      .then((_) => this.dwtService.unMountVideoContainer());
  }
  clearMessage() {
    this.instantError = '';
  }
  ...
  toggleCheckAll() {
    ...
    for (let key in _symbologies) {
      if (_symbologies.hasOwnProperty(key)) {
        if (key !== 'All')
          _symbologies[key as keyof typeof _symbologies] = !_symbologies['All'];
      }
    }
  }
  showRects() {
    ...
    this.barcodeRectsOnCurrentImage = [];
    if (
      this.barcodeRects === undefined ||
      this.barcodeRects.imageIds === undefined
    )
      return;
    if (
      this.barcodeRects.imageIds.length === 0 ||
      this.barcodeRects.imageIds.length !== this.barcodeRects.rects.length
    )
      //invalid rects
      return;
    for (let i = 0; i < this.barcodeRects.imageIds.length; i++) {
      let currentIndex = this.DWTObject?.ImageIDToIndex(
        this.barcodeRects.imageIds[i]
      );
      if (this.DWTObject?.CurrentImageIndexInBuffer === currentIndex) {
        let rectsOnOnePage = this.barcodeRects.rects[i];
        let mainViewer = <HTMLDivElement>(
          document.querySelector('#' + this.containerId + ' .dvs-viewer-main')
        );
        let zoom = 0,
        ...
        for (let j = 0; j < rectsOnOnePage.length; ++j) {
          ...
          this.barcodeRectsOnCurrentImage.push({
            x: left,
            y: top,
            w: width,
            h: height,
  ...
  unBindViewer() {
    if (this.DWTObject?.Viewer.unbind()) {
      ...
      if (!this.emptyBuffer) this.clearMessage();
      return true;
    } else {
  ...
  bindViewer() {
    ...
    if (this.thumbnail) {
      ...
      this.DWTObject.Viewer.on('pageAreaSelected', (nImageIndex, rect) => {
        if (rect.length > 0) {
          ...
          if (this.zones.length + 1 === rect.length)
            ...
            this.zones.splice(rect.length - 1, 1, {
              x: currentRect.x,
              y: currentRect.y,
              width: currentRect.x + currentRect.width,
              height: currentRect.y + currentRect.height,
      ...
      //this.DWTObject.Viewer.off('imageRightClick');
      this.bMobile
        ? this.thumbnail.updateViewMode(<ViewMode>{ columns: 1, rows: 5 })
      ...
      WASMInput.onchange = (evt) => {
        ...
        this.dwtService.load(_input.files!).then((_) => {
      ...
      document
        ...
        ?.parentElement?.appendChild(WASMInput);
    } else {
      console.log(this.DWTObject.ErrorString);
    }
  }
  initDWT(): void {
    this.DWTObject = null as unknown as WebTwain;
    this.dwtService.mountDWT().then(
      (obj) => {
        this.DWTObject = obj;
        ...
        this.dwtService.mountVideoContainer().then(
          (containerDWT) => {
        ...
        setTimeout(() => {
          this.bindViewer();
          if (this.DWTObject) this.DWTObject.Viewer.pageMargin = 10;
        }, 0);
      },
      (err) => this.showMessage(err)
    );
  ...
  openModal(content: any, type?: string) {
        ...
        switch (type) {
          ...
          case 'camera':
            if (this.videoPlaying) {
              ...
              this.deviceName = 'Choose...';
              return this.VideoContainer?.Viewer.unbind() as boolean;
            }
          case 'barcode':
    ...
    this.modalRef.result.then(
      ...
      (reason) => {
        //dismiss, reset modal dialogs
        if (this.emptyBuffer) this.showMessage('There is no image in buffer!');
        this.saveResults = {
          blob: [],
          blobURL: [],
          base64String: [],
    ...
    switch (type) {
      case 'camera':
        let makeSureDIVExists = () => {
          let container = <HTMLDivElement>(
            document.getElementById(this.videoContainerId)
          );
          if (container) {
            ...
            this.VideoContainer.Viewer.bind(
              <HTMLDivElement>document.getElementById(this.videoContainerId)
            );
            this.VideoContainer.Viewer.width = '100%';
            this.VideoContainer.Viewer.height = '100%';
            this.VideoContainer.Viewer.show();
          } else setTimeout(() => makeSureDIVExists(), 10);
        };
        makeSureDIVExists();
        this.clearMessage();
        this.dwtService.getDevices(true).then(
          (result) => {
            this.devices = result;
      ...
      case 'acquire':
        this.clearMessage();
        this.dwtService.getDevices(false).then(
          (result) => {
            this.devices = result;
            this.showDevices = true;
          },
      ...
      case 'save':
        ...
        for (let i = 0; i < count!; i++)
          (this.saveOptions.indices as any[]).push({
            number: i,
            selected: !!selectedIndices?.find((o) => {
              return o == i;
            }),
          });
        break;
  ...
  closeModal(close: boolean) {
    ...
    if (close) this.modalRef.close();
    else this.modalRef.dismiss();
  }
  handleDeviceChange(deviceType: string) {
    if (this.deviceName === '' || this.deviceName === 'Choose...') return;
    this.dwtService.selectADevice(this.deviceName).then(
      (done) =>
        done
          ? this.openADevice(deviceType)
          : this.showMessage('Device selecting failed!'),
      (error) =>
        typeof error === 'string'
  ...
  openADevice(deviceType: string) {
    ...
    if (deviceType === 'camera') {
      if (this.videoPlaying) this.toggleVideo();
      this.toggleVideo();
    }
  }
  acquire() {
  ...
  acquireFromCamera() {
    this.dwtService.acquire().then(
      (_) => {
        this.showMessage('Captured successfully!');
        setTimeout(() => {
          this.clearMessage();
        }, 3000);
      },
  ...
  scan() {
    this.dwtService.acquire(this.scanOptions).then(
      () => {
        this.closeModal(true);
        if (!this.emptyBuffer) this.clearMessage();
      },
  ...
  readBarcode() {
    ...
    let formatId = 0,
      formatId2 = 0;
    this.barcodeReaderOptions.Symbologies.Aztec
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_AZTEC)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.DM
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_DATAMATRIX)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.Dot
      ? (formatId2 += Dynamsoft.DBR.EnumBarcodeFormat_2.BF2_DOTCODE)
      : (formatId2 += 0);
    this.barcodeReaderOptions.Symbologies.GS1Composite
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_GS1_COMPOSITE)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.GS1Databar
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_GS1_DATABAR)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.Linear
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_ONED)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.Maxi
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_MAXICODE)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.Patch
      ? (formatId += Dynamsoft.DBR.EnumBarcodeFormat.BF_PATCHCODE)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.Pdf
      ? (formatId +=
          Dynamsoft.DBR.EnumBarcodeFormat.BF_PDF417 +
          Dynamsoft.DBR.EnumBarcodeFormat.BF_MICRO_PDF417)
      : (formatId += 0);
    this.barcodeReaderOptions.Symbologies.QRCode
      ? (formatId +=
    ...
    this.barcodeReaderOptions.Symbologies.Postal
      ...
      : (formatId2 += 0);
    this.barcodeRectsOnCurrentImage.splice(
      0,
      this.barcodeRectsOnCurrentImage.length
    );
    this.dwtService.readBarcode({
      formatId: formatId,
      formatId2: formatId2,
      zones: this.zones,
    });
  ...
  handleOutPutMessage(
    ...
    if (_type === 'httpResponse') {
      ...
      (msgWindow as any).document.writeln(message);
    } else {
  ...
  setUpPlayVideo(config?: any) {
    let _dwt = this.DWTObject;
    if (this.VideoContainer) _dwt = this.VideoContainer;
    ...
    if (config) {
      ...
      if (config.prop === 'Video Setup' || config.prop === 'Camera Setup') {
        ...
        if (config.prop === 'Video Setup') {
          bCamera = false;
          basicSetting = _dwt.Addon.Webcam.GetVideoPropertySetting(
            Dynamsoft.DWT.EnumDWT_VideoProperty[
              ('VP_' + config.value) as unknown as number
            ] as unknown as number
          );
          moreSetting = _dwt.Addon.Webcam.GetVideoPropertyMoreSetting(
            Dynamsoft.DWT.EnumDWT_VideoProperty[
              ('VP_' + config.value) as unknown as number
            ] as unknown as number
          );
        } else {
          basicSetting = _dwt.Addon.Webcam.GetCameraControlPropertySetting(
            Dynamsoft.DWT.EnumDWT_CameraControlProperty[
              ('CCP_' + config.value) as unknown as number
            ] as unknown as number
          );
          moreSetting = _dwt.Addon.Webcam.GetCameraControlPropertyMoreSetting(
            Dynamsoft.DWT.EnumDWT_CameraControlProperty[
              ('CCP_' + config.value) as unknown as number
            ] as unknown as number
          );
        ...
        switch (config.prop) {
          case 'Frame Rate':
            _dwt.Addon.Webcam.SetFrameRate(config.value);
            break;
          case 'Media Type':
            _dwt.Addon.Webcam.SetMediaType(config.value);
            break;
          case 'Resolution':
            _dwt.Addon.Webcam.SetResolution(config.value);
            break;
          default:
            break;
        }
  ...
  toggleVideo() {
    ...
    if (this.videoPlaying) {
      ...
      if (this.bUseCameraViaDirectShow) return _dwt.Addon.Webcam.StopVideo();
    } else return this.playVideo();
  ...
  handleRangeReset() {
    ...
    this.rangePicker.bCamera
      ? _dwt.Addon.Webcam.SetCameraControlPropertySetting(
          Dynamsoft.DWT.EnumDWT_CameraControlProperty[
            ('CCP_' + this.rangePicker.title) as unknown as number
          ] as unknown as number,
          this.rangePicker.default,
          true
        )
      : _dwt.Addon.Webcam.SetVideoPropertySetting(
          Dynamsoft.DWT.EnumDWT_VideoProperty[
            ('VP_' + this.rangePicker.title) as unknown as number
          ] as unknown as number,
          this.rangePicker.default,
          true
        );
  ...
  outPutTypeChanged(newType: any) {
    switch (newType) {
      case 'File':
        this.saveOptions.upload
          ? (this.saveOptions.buttonText = 'Upload')
          : (this.saveOptions.buttonText = 'Save');
        break;
      case 'Blob':
  ...
  getImageType(formatString: string): number {
    ...
    Object.entries(Dynamsoft.DWT.EnumDWT_ImageType).forEach((_type) => {
      if (_type[0] === formatString) imageType = <number>_type[1];
    });
    return imageType;
  }
  copyBase64String(indexOfString: any) {
    if (navigator.clipboard) {
      if (this.saveResults.base64String[indexOfString] === '')
        this.showMessage('No resulting String!');
      navigator.clipboard
        .writeText(this.saveResults.base64String[indexOfString])
        .then((_) => {
  ...
  copyFilePath(indexOfString: any) {
    if (navigator.clipboard) {
      if (this.saveResults.saveFileText[indexOfString] === '')
        this.showMessage('No resulting String!');
      navigator.clipboard
        .writeText(this.saveResults.savedFiles[indexOfString].path)
        .then((_) => {
  ...
  handleMultiPageCheck() {
    if (this.saveOptions.multiPage) {
      if (this.DWTObject.SelectedImagesIndices.length === 1) {
        this.saveOptions.indices.forEach(
  ...
  handleOutPutFormatChange(format: any) {
    ...
    switch (this.saveOptions.outPutType) {
            ...
            for (let i = 0; i < count; i++) {
              this.dwtService
                .uploadToServer(
                  [i],
                  this.getImageType(this.saveOptions.outPutFormat),
                  this.saveOptions.fileName + '_' + (i + 1)
                )
                .then(
                  (result) => {
                    this.saveResults.uploadedFiles.push(result);
            ...
            this.dwtService
              ...
              .then(
                (result) => {
                  ...
                  this.saveResults.saveFileText.push(
                    'Copy path for ' + result.name
                  );
                  this.clearMessage();
                },
                (err) => this.showMessage(err)
      ...
      case 'Blob':
        if (this.saveOptions.multiPage) {
          ...
          for (let i = 0; i < count; i++) {
            this.dwtService
              .getBlob([i], this.getImageType(this.saveOptions.outPutFormat))
              .then(
                (blob) => {
                  let newFile = new File(
                    [blob],
                    'Saved_Blob_' + (i + 1) + '(' + blob.type + ')',
                    { type: blob.type }
                  );
                  this.saveResults.blob.push(newFile);
                  this.saveResults.blobURL.push(URL.createObjectURL(newFile));
      ...
      case 'Base64':
        if (this.saveOptions.multiPage) {
          ...
          for (let i = 0; i < count; i++) {
            this.dwtService
              .getBase64([i], this.getImageType(this.saveOptions.outPutFormat))
              .then(
                (base64String) => {
                  this.saveResults.base64String.push(base64String);
                  this.saveResults.base64ButtonText.push(
                    'Copy Base64 String for image ' + i
                  );
                  this.clearMessage();
                },
                (err) => this.showMessage(err)
  ...
  copyURLToShow(url: string) {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then((_) => {
        this.showMessage(
          "URL of the blob copied, try paste it in another tab to view it or download the blob as a file. Don't forget to add an extension to the downloaded file before opening it!"
        );
      });
    } else {
      this.showMessage('Can not use the clipboard! Please use HTTPS.');
