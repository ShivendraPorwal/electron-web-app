...
import {
  ...
  TextResults,
  TextResult,
} from 'dwt/dist/types/Addon.BarcodeReader';
...
export class DwtService {
   ...
   * WebTwain objects
   */
  protected _DWTObject: WebTwain = NULL_WebTwain;
  protected _DWTObjectEx: WebTwain = NULL_WebTwain;
  /**
  ...
  public barcodeRects: any = { imageIds: [], rects: [] };
  /**
   * Save
   */
  private fileSavingPath = 'C:';
  private fileActualName = '';
  private _doFunction: any;
  ...
  constructor() {
    ...
    Dynamsoft.DWT.Containers = [
      { WebTwainId: 'dwtcontrolContainer', Width: 270, Height: 350 },
    ];
    Dynamsoft.DWT.Load();
    /**
  ...
  unMountDWT(): Promise<any> {
    return new Promise((res, rej) => {
      if (Dynamsoft.DWT.DeleteDWTObject('dwtObject')) res(true);
      else rej(false);
    });
  }
  /**
  ...
  mountVideoContainer(): Promise<any> {
    this._DWTObjectEx = NULL_WebTwain;
    return new Promise((res, rej) => {
      if (this._DWTObject) {
        let dwtInitialConfig: DWTInitialConfig = {
  ...
  unMountVideoContainer(): Promise<any> {
    return new Promise((res, rej) => {
      if (Dynamsoft.DWT.DeleteDWTObject('videoContainer')) res(true);
      else rej(false);
    });
  }
  /**
   ...
   */
  getDevices(bfromCamera: any): Promise<Device[]> {
    return new Promise((res, rej) => {
      let _dwt = this._DWTObject;
      if (this._DWTObjectEx) _dwt = this._DWTObjectEx;
      this.devices = [];
      this._DWTObject
        .GetDevicesAsync()
        .then((devicesList) => {
          for (let i = 0; i < devicesList.length; i++) {
            this.devices.push({
              ...
              name: (i + 1).toString() + '.' + devicesList[i].displayName,
          ...
          if (this.bUseCameraViaDirectShow) {
            try {
              ...
              if (bfromCamera) rej(e);
              else {
                if (e.code == -2338) res(this.devices);
                else rej(e);
              }
            }
          }
  ...
  selectADevice(name: string): Promise<boolean> {
    return new Promise((res, rej) => {
      ...
      if (this.devices.length === 0) rej(false);
  ...
  updateCameraValues(_dwt: WebTwain) {
    ...
    let _advancedSettings: any = [],
      _advancedCameraSettings: any = [];
    for (let i = 0; i < mediaTypes.GetCount(); i++) {
      mediaTypes.Get(i) === _currentmT
        ? (_mediaTypes[i] = {
            value: mediaTypes.Get(i).toString(),
            checked: true,
          })
        : (_mediaTypes[i] = {
            value: mediaTypes.Get(i).toString(),
    ...
    for (let i = 0; i < frameRates.GetCount(); i++) {
      frameRates.Get(i) === _currentfR
        ? (_frameRates[i] = {
            value: frameRates.Get(i).toString(),
            checked: true,
          })
        : (_frameRates[i] = {
            value: frameRates.Get(i).toString(),
    ...
    for (let i = 0; i < resolutions.GetCount(); i++) {
      resolutions.Get(i) === _currentRes
        ? (_resolutions[i] = {
            value: resolutions.Get(i).toString(),
            checked: true,
          })
        : (_resolutions[i] = {
            value: resolutions.Get(i).toString(),
    ...
    _advancedSettings = Object.keys(Dynamsoft.DWT.EnumDWT_VideoProperty).map(
      (_value) => {
              ...
              this._DWTObjectEx.Addon.Webcam.CaptureImage(
                () => {
                  this.getBlob(
                    ...
                    this._DWTObject.LoadImageFromBinary(
                      blob,
                      () => {
  ...
  filterZones(index: number, zones: Zone[]): Zone[] {
    if (zones !== undefined && zones.length > 0) {
      for (let i = 0; i < zones.length; i++) {
        if (zones[i].index !== index) zones.splice(i, 1);
      }
      return zones;
    }
    return [];
  ...
  readBarcode(config?: any) {
    ...
    this._DWTObject.Addon.BarcodeReader.getRuntimeSettings().then(
      (settings) => {
        ...
        if (config && config.zones && config.zones.length > 0) {
          ...
          let readBarcodeFromRect = () => {
            ...
            settings.region.regionRight =
              config.zones[i].x + config.zones[i].width;
            settings.region.regionBottom =
              config.zones[i].y + config.zones[i].height;
            if (i === config.zones.length - 1)
              this.doReadBarode(_index, settings, () => {});
            else this.doReadBarode(_index, settings, readBarcodeFromRect);
          };
          settings.region.regionLeft = config.zones[0].x;
          settings.region.regionTop = config.zones[0].y;
          settings.region.regionRight =
            config.zones[0].x + config.zones[0].width;
          settings.region.regionBottom =
            config.zones[0].y + config.zones[0].height;
  ...
  doReadBarode(index: number, settings: RuntimeSettings, callback: () => void) {
    ...
    let outputResults = () => {
      ...
      if (this.barcodeResults.length === 0) {
        ...
        resultString.push({
          text: 'Nothing found on the image!',
          type: 'important',
        });
        ...
        resultString.push({
          text: 'Total barcode(s) found: ' + allBarcodeResults.length,
          type: 'important',
        });
    ...
    this._DWTObject.Addon.BarcodeReader.updateRuntimeSettings(settings).then(
      (_) => {
        let decoderFunc = () => {
          try {
            this._DWTObject.Addon.BarcodeReader.decode(index).then(
              (textResults) => {
  ...
  returnBarcodeRects() {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (existingIndex === -1) {
        ...
        this.barcodeRects.rects[existingIndex] =
          this.barcodeRects.rects[existingIndex].concat(tempRects);
      }
    }
      ...
      let saveInner = (_path: any, _name: any, _type: any): Promise<any> => {
        return new Promise((res, rej) => {
          let s = () => {
            ...
            f = (errCode: any, errStr: any) => rej(errStr);
          switch (_type) {
            case 0:
  ...
  uploadToServer(
    ...
    return new Promise((res, rej) => {
      ...
      if (environment.Dynamsoft.uploadTargetURL !== '') {
        ...
        savedDir =
          protocol + window.location.hostname + ':' + _strPort + '/uploaded/';
        url =
